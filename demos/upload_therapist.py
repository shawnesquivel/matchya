...
def upload_therapist(embedding, metadata):
    ...
    try:
        ...
        error = f"there was an error uploading the therapist id"
...
def check_link_in_directory():
    """Returns true if the object is found in the directory."""
    return
...
def upload_therapist_directory(links: list[str]):
    errors = []
    website_docs = bs(links)
    for i, website in enumerate(website_docs):
        therapist_json_string = extract_json(website.page_content)
        try:
            therapist_json = json.loads(therapist_json_string)
            therapist_json["bio_link"] = links[i]
            print(f"JSON: {therapist_json}")
        except Exception as e:
            print("could not load json, skipping therapist.")
            errors.append({"error": str(e), "link": links[i]})
            continue

        # print(therapist_json)
        try:
            html = fetch_raw_html(links[i])
            ...
            if not therapist_json["profile_link"].startswith("https://"):
                errors.append(
                    {
                        "error": "profile link dies not start with https",
                        "link": links[i],
                    }
                )
                raise ValueError("Invalid profile link")
        except Exception as e:
            print("could not extract image link, setting to None")
        ...
        try:
            ...
            print(f"AI Summary: {summary[0:10]}")
        except Exception as e:
        ...
        try:
            ...
            print(f"AI Short Summary: {summary[0:10]}")
        except Exception as e:
            therapist_json["summary"] = None
            errors.append({"error": str(e), "link": links[i]})
