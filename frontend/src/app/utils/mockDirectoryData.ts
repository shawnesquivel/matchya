...
const createMockTherapist = (id: string, firstName: string, lastName: string, city: string, province: string): Therapist => {
  return {
    id,
    first_name: firstName,
    last_name: lastName,
    gender: ["female", "male", "non_binary"][parseInt(id) % 3] as "female" | "male" | "non_binary",
    ethnicity: ["Caucasian", "Asian", "Black", "Hispanic"].slice(0, 1 + (parseInt(id) % 3)),
    sexuality: ["Straight", "LGBTQ+"].slice(0, 1 + (parseInt(id) % 2)),
    faith: ["Christian", "Buddhist", "Muslim", "Jewish", "Hindu", "Atheist"].slice(0, 1 + (parseInt(id) % 3)),
    clinic_city: city,
    clinic_province: province,
    clinic_country: province === "ON" || province === "BC" ? "Canada" : "United States",
    ...
    languages: ["English", "Spanish", "French"].slice(0, 1 + (parseInt(id) % 3)),
    certifications: [`Certified in ${getRandomSpecialties(1)[0]}`, `Advanced training in ${getRandomSpecialties(1)[0]}`],
    profile_img_url: `/assets/images/mock/therapist-${(parseInt(id) % 8) + 1}.jpg`,
    bio: `${firstName} is a licensed therapist based in ${city}, with over ${5 + (parseInt(id) % 10)} years of experience helping clients achieve mental wellness and personal growth. Specializing in ${getRandomSpecialties(2).join(" and ")}, ${firstName} provides compassionate and effective therapy for individuals of all backgrounds.`,
    ...
    therapist_email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}@example.com`,
    initial_price: `${120 + (parseInt(id) % 5) * 10}`,
    subsequent_price: `${100 + (parseInt(id) % 5) * 10}`,
    availability: ["both", "online", "in_person"][parseInt(id) % 3] as "both" | "online" | "in_person",
    clinic_profile_url: `https://example.com/therapists/${firstName.toLowerCase()}-${lastName.toLowerCase()}`,
    clinic_booking_url: `https://example.com/book/${firstName.toLowerCase()}-${lastName.toLowerCase()}`,
...
const getRandomAreasOfFocus = (count: number): string[] => {
  const areas = [
    ...
    "Grief & Loss", "Life Transitions", "Self-Esteem", "Career Counseling",
    "Family Conflict", "PTSD", "Addiction", "ADHD", "Anger Management",
    "Bipolar Disorder", "Chronic Illness", "Divorce", "Eating Disorders",
    "Identity Development", "Parenting", "Sleep Issues", "Work-Life Balance"
  ];
...
const getRandomApproaches = (count: number): string[] => {
  const approaches = [
    "Cognitive Behavioral Therapy (CBT)", "Psychodynamic", "Mindfulness-Based",
    "Solution-Focused", "Narrative Therapy", "Humanistic", "Acceptance & Commitment (ACT)",
    "Dialectical Behavior Therapy (DBT)", "Motivational Interviewing",
    "Eye Movement Desensitization and Reprocessing (EMDR)", "Family Systems",
    "Person-Centered", "Psychoanalytic", "Gestalt", "Emotion-Focused"
  ];
...
const getRandomSpecialties = (count: number): string[] => {
  const specialties = [
    "anxiety disorders", "depression", "trauma recovery", "relationship issues",
    "stress management", "grief counseling", "addiction recovery", "career development",
    "family therapy", "couples counseling", "mindfulness practices", "cognitive behavioral techniques"
  ];
...
const getRandomUniversity = (): string => {
  const universities = [
    ...
    "Stanford University", "University of Michigan", "Columbia University"
  ];
...
const canadaTherapists: Therapist[] = [
  // Ontario therapists
  createMockTherapist("1001", "Sarah", "Johnson", "Toronto", "ON"),
  createMockTherapist("1002", "Michael", "Chen", "Toronto", "ON"),
...
const mockCities = {
  ca: {
    ...
    bc: ["Vancouver", "Victoria", "Surrey", "Burnaby", "Kelowna", "Abbotsford", "Richmond"]
  },
  us: {
    ca: ["Los Angeles", "San Francisco", "San Diego", "San Jose", "Oakland", "Sacramento", "Fresno", "Long Beach"],
    ny: ["New York City", "Brooklyn", "Queens", "Buffalo", "Rochester", "Syracuse", "Albany", "Yonkers"]
  }
};

// Utility functions to get mock data
export const getMockTherapistsByCountry = (country: string, page: number = 1, pageSize: number = 20, name?: string): { therapists: Therapist[], totalCount: number } => {
  const therapists = country.toLowerCase() === "ca" ? canadaTherapists : usTherapists;
...
export const getMockTherapistsByRegion = (country: string, region: string, page: number = 1, pageSize: number = 20, name?: string): { therapists: Therapist[], totalCount: number } => {
  const allTherapists = country.toLowerCase() === "ca" ? canadaTherapists : usTherapists;
...
export const getMockTherapistsByCity = (country: string, region: string, city: string, page: number = 1, pageSize: number = 20, name?: string): { therapists: Therapist[], totalCount: number } => {
  const allTherapists = country.toLowerCase() === "ca" ? canadaTherapists : usTherapists;
...
export const getMockRegionsByCountry = (country: string): { code: string, name: string }[] => {
  const countryCode = country.toLowerCase();
  return countryCode in mockRegions ? mockRegions[countryCode as keyof typeof mockRegions] : [];
};

export const getMockCitiesByRegion = (country: string, region: string): string[] => {
  const countryCode = country.toLowerCase();
